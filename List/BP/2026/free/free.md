...
- 자유는 emergent property이다: 자유란 \<주어진 시스템이 metastable state에 갖히지 않고 다른 state로 전이될 probability sturucture\>로 정의됨
...
- 자유는 궤적의 이탈 가능성과 관련되어 있음
    - 자유의 정의는 (2), (3)이지만 자유의 실현 조건은 (1)임.
        - 남한: ocal minima가 무수히 많은데 global optimum이 없음, local optimum path가 거의 결정되어 있음.
...
- 선택지 자체는 많은데 실제로 사람들이 하는 선택들은 정해져 있다면, 형식적 자유는 있는데 실질적 자유가 없단 소리임. 하지만 진정한 의미의 자유는 둘 모두의 capacity를 만족해야 함
    ...
    - 게임이론 비유: 에이전트들이 전략을 자유롭게 선택은 하는데, 이미 payoff landscape의 geometry가 디자인돼 있어서 에이전트들이 실제로 방문하는 상태가 거의 결정돼 있고, 내쉬 균형은 존재하는데 거기에 도달하는 모든 경로들이 사실상 비슷하다는 것임.
